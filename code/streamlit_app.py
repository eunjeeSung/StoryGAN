...
device = "cuda"
desc_dir = '/home/ubuntu/StoryGAN/vist_dataset/sample_annotation'
video_len = 5
...
with st.beta_expander("Make your own storybook!"):
    ...
    encoded_descriptions = \
        torch.load(os.path.join(desc_dir,
                    'sample_test_encodings.pt'))[:5, :]

# Run the selected poem through the model
args = setup()
fake_imgs = single_inference(args, encoded_descriptions)
...
st.sidebar.markdown(
    ...
    f" The model was trained on the VIST dataset from Microsoft."
)
st.sidebar.markdown(
